...
import MonopolyLinearPageStyle from "./MonopolyLinearPage.module.css"
...
const MonopolyLinearPage = ({navigateTo, backgroundImage,currentProblemIndex,setCurrentProblemIndex,players,setPlayers,bgmAudio}) => {
    ...
    const handleAnswer=(button,optionText)=>{
        if(modeQuestions[currentProblemIndex]?.answer===optionText){
           ...
           // 答錯退回上一個位置 (lastPosition)
            const playerId = players.find(p => p.current === true).id;
            const prevStep = lastPosition[playerId] || 1; // 找不到則回原點
            
            // 計算退回所需的相對步數 (prevStep - currentStep)
    ...
    const renderPieces = (stepNum) => {
        ...
        // 根據 stepNum 決定容器的 className
        let containerClassName = '';
        if (stepNum === 1) {
            containerClassName = ''; // 原本邏輯為空字串
        } else if (stepNum === 8 || stepNum === 11) {
            containerClassName = MonopolyLinearPageStyle[`player-pieces-container-with-img-${stepNum}`];
        } else {
            containerClassName = MonopolyLinearPageStyle['player-pieces-container'];
        }
    ...
    const handleOpenChest=()=>{// 打開命運卡
        return new Promise((resolve,reject)=>{
            ...
            setTimeout(()=>{
                ...
                if(cardId===1){// 後退一步
                    const playerId=players.find(p=>p.current===true).id
                    handleMoveThePlayer(playerId,8,-1,true)
                    reject()
                }
                else if(cardId===2){// 回到原點
                    const playerId=players.find(p=>p.current===true).id
                    handleMoveThePlayer(playerId,8,-100,true)
                }
    ...
    return (
        <div className="page-container" style={pageStyle}>
            {/* 卡片區域：寶箱 */}
            <div className={`${MonopolyLinearPageStyle["card-section"]} ${sectionVisible.chest === false ? MonopolyLinearPageStyle.sectionHidden : ''}`}>
                <img src={`./images/object/Basketball_monopoly_community_chest_card_0${cardIndex.chest}.png`} alt={`chest_card_0${cardIndex.chest}`} />
            </div>

            {/* 卡片區域：機會 */}
            <div className={`${MonopolyLinearPageStyle["card-section"]} ${sectionVisible.chance === false ? MonopolyLinearPageStyle.sectionHidden : ''}`}>
                <img src={`./images/object/Basketball_monopoly_chance_card_0${cardIndex.chance}.png`} alt={`chance_card_0${cardIndex.chance}`} />
            </div>

            {/* 骰子區域 */}
            <div className={`${MonopolyLinearPageStyle["dice-section"]} ${sectionVisible.dice === false ? MonopolyLinearPageStyle.sectionHidden : ''}`}
                ...
                <div className={MonopolyLinearPageStyle["dice-face"]} style={{ cursor: isRolling ? "not-allowed" : "pointer" }}>
                    <div className={`${MonopolyLinearPageStyle["dice-dots"]} ${MonopolyLinearPageStyle[`dice-value-${diceNumber}`]}`}>
                        {/* 根據點數產生對應數量的點點 */}
                        {Array.from({ length: diceNumber }).map((_, i) => (
                            <span key={i} className={MonopolyLinearPageStyle.dot}></span>
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div>
                {Array.from({ length: 23 }, (_, i) => i + 1).map((stepNum) => (
                    <div key={stepNum} className={`${MonopolyLinearPageStyle["step-box"]} ${MonopolyLinearPageStyle[`step-${stepNum}`]}`}>
                        {stepNum === 1 && <img src="./images/object/Basketball_monopoly_GO.png" alt="" className={MonopolyLinearPageStyle["grid-icon"]} />}
                        {stepNum === 8 && <img src="./images/object/Basketball_monopoly_question_mark.png" alt="" className={MonopolyLinearPageStyle["grid-icon"]} />}
                        {stepNum === 11 && <img src="./images/object/Basketball_monopoly_treasure_chest.png" alt="" className={MonopolyLinearPageStyle["grid-icon"]} />}
...
export default MonopolyLinearPage;